...
import FindExpertFlow from "./FindExpertFlow";
...
// FIX: Receive history and setHistory as props from App.js
export default function Experts({ history, setHistory }) {
  const [finderOpen, setFinderOpen] = useState(false);
  // const [history, setHistory] = useState([]); <--- हे काढून टाकले आहे कारण आता हे App.js मधून येते.
  ...
  const handleUpdateRequest = (updatedItem) => {
    // 1. लिस्ट मध्ये जुन्या ID चा आयटम शोधून त्याला नवीन डेटाने अपडेट करणे
    setHistory(prev => prev.map(item => item.id === updatedItem.id ? updatedItem : item));
    
    // 2. अपडेट झाल्यावर Detail Page बंद करून परत Experts List वर येणे
    setSelected(null); 
  };
  ...
  // Handle case where history might be undefined initially (safety check)
  const safeHistory = history || [];

  const filtered =
    activeTab === "All"
      ? safeHistory
      : safeHistory.filter((h) => h.status === activeTab);
  ...
  return (
    ...
    >
      ...
      <FindExpertFlow
        ...
        onComplete={(entry) =>
          setHistory([
            ...safeHistory, // Use safeHistory here
            {
              id: Date.now(),
              area: entry.area, 
