...
import RejectExpertModal from './RejectExpertModal'
...
export default function ExpertDetailPage({ data, onBack, onDelete, onUpdate }) {
...
const [isRejectModalOpen, setIsRejectModalOpen] = useState(false);
    ...
    const handleConfirmYes = () => { setIsConfirmOpen(false); setIsSuccessOpen(true); };
    const handleSuccessOk = () => { setIsSuccessOpen(false); if(onDelete) onDelete(data.id); };
    const handleAcceptClick = () => setIsAcceptOpen(true);
    const handleDashboardRedirect = () => { setIsAcceptOpen(false); navigate("/dashboard"); };
const handleRejectClick = () => {
    ...
    const handleRejectConfirm = (reason) => {
        ...
        // Update Status to "Rejected" in Parent Component (Experts.jsx)
        if (onUpdate) {
            onUpdate({
                ...data,
                status: "Rejected",
                statusText: "Your consultant has made an engagement plan for your business. Review the plan details to begin next step", // As per PDF Frame 1984078928
                rejectReason: reason
            });
        }
    };

    const handleRejectSuccessDismiss = () => {
        setIsRejectSuccessOpen(false);
        // "Back to dashboard" logic -> Close Detail View (Go back to list)
        if (onBack) onBack(); 
    };
    return (
        <div className="w-full h-screen bg-[#F9FAFB] md:p-8 font-plex-hebrew flex justify-center overflow-hidden">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[35%_65%] gap-8 h-full overflow-hidden">
                    ...
                    <div className="flex flex-col h-full overflow-y-auto custom-scrollbar pr-2">
                        ...
                        <div className="items-center gap-4 mt-auto pb-4" style={{ textAlign: 'center' }}>
                            <button 
                                ...
                                className="flex items-center justify-center w-full border border-[#2D60FF] p-3 bg-[#F3F9FF] rounded-xl text-[#2D60FF] gap-2 text-[14px] font-semibold hover:text-[#2D60FF] transition-colors"
                            >
                                Edit request
                            </button>
                            <button 
                    ...
                    <div className="flex flex-col h-full border border-[#D1E9FF] rounded-2xl overflow-hidden bg-white">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                            ...
                            {/* 2. PROMO CARD (NOW AT BOTTOM) */}
                            <div className="w-full bg-[#EFF8FF] rounded-xl p-5 border border-blue-50">
                        ...
                        {showExpert && (
                            <div className="p-4 border-t border-gray-200 bg-white shrink-0 flex justify-end gap-3 z-10 shadow-[0_-4px_10px_-4px_rgba(0,0,0,0.05)]">
                                <button className="w-24 py-1.5 border border-gray-300 text-gray-700 rounded-lg font-semibold text-[13px] hover:bg-gray-50 hover:text-red-600 transition-colors" onClick={handleRejectClick}>
                                    Reject
                                </button>
                                <button 
            ...
            {/* --- MODALS --- */}
            <DeleteModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={handleConfirmYes} />
            <DeleteSuccessModal isOpen={isSuccessOpen} onOk={handleSuccessOk} />
            <AcceptSuccessModal isOpen={isAcceptOpen} onDashboard={handleDashboardRedirect} />
            ...
            <RejectSuccessModal 
                ...
                onDashboard={handleRejectSuccessDismiss} 
            />
